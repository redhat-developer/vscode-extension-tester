...
describe('EditorView', function () {
    ...
    it('openEditor works with webview editor', async function () {
        ...
        chai.expect(editor.findWebElement).not.undefined;
    ...
    it('openEditor works with diff editor', async function () {
        ...
        chai.expect(await diffEditor.getOriginalEditor()).not.undefined;
        chai.expect(await diffEditor.getModifiedEditor()).not.undefined;
    });

    it('getTabByTitle works', async function () {
        const tab = await view.getTabByTitle('Untitled-1');
        chai.expect(tab).not.undefined;
    });

    it('getOpenEditorTitles works', async function () {
        const tabs = await view.getOpenEditorTitles();
        chai.expect(tabs).not.empty;
        chai.expect(tabs).contains('Untitled-1');
        chai.expect(tabs).contains('Untitled-2');
    });

    it('closeEditor works', async function () {
        await view.closeEditor('Untitled-1');
        const tabs = await view.getOpenEditorTitles();
        chai.expect(tabs).not.contains('Untitled-1');
    });
    ...
    it('getAction(title: string) works', async function () {
        ...
        chai.expect(await action.getTitle()).equal('More Actions...');
    });
    ...
    it('Editor getAction works', async function () {
        ...
        chai.expect(editorAction).not.undefined;
    });
    ...
    describe('Editor Tab', function () {
        ...
        it('getTitle works', async function () {
            chai.expect(await tab2.getTitle()).equals('Untitled-2');
        });

        it('isSelected works on active tab', async function () {
            chai.expect(await tab3.isSelected()).to.be.true;
        });

        it('isSelected works on inactive tab', async function () {
            chai.expect(await tab2.isSelected()).to.be.false;
        });
    });
    ...
    describe('Editor Groups', function () {
        ...
        it('getEditorGroups works', async function () {
            ...
            chai.expect(groups.length).equals(2);
            chai.expect((await group1.getRect()).x).equals((await groups[0].getRect()).x);
            chai.expect((await group2.getRect()).x).equals((await groups[1].getRect()).x);
        });
        ...
        it('openEditor works for different groups', async function () {
            ...
            chai.expect((await editor1.getRect()).x < (await editor2.getRect()).x);
        });
        ...
        it('closeEditor works for different groups', async function () {
            ...
            chai.expect((await view.getEditorGroups()).length).equals(2);
        });
        ...
        it('getOpenEditorTitles works for different editor groups', async function () {
            ...
            chai.expect(titles).deep.equals(allTitles);
        });
    });
