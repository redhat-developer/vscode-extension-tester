...
ExTester **currently supports the latest 3 minor releases** of VS Code (subject to change if VS Code 2.x ever comes out). Additionally, the oldest release of VS Code that can be used with ExTester is `1.90.0` — older versions ship a Chromium whose ChromeDriver predates Chrome for Testing and can no longer be provisioned.

Versions older than the supported 3 latest releases, but still newer than `1.90.0` will most likely work just fine. However, if they stop working over time, there will be no more fixes to make them work. Unless you'd like to contribute these.
