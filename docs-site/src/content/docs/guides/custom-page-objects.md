...
A path passed via the CLI flag or `RunOptions` is resolved relative to the current working directory. A `run.customPageObjects` path declared inside `extester.config.json` is resolved relative to that config file's directory.
...
The option can also be set via `run.customPageObjects` in `extester.config.json` — see [Using a Config File](/vscode-extension-tester/guides/test-setup/#using-a-config-file):

```json
{
  "run": {
    "customPageObjects": "./out/test/pageObjects/locators.js"
  }
}
```
...
- The locator contribution file must export a property named `locators` at the module root. Note that the built-in version-specific locator files export `diff` — but custom contribution files must export `locators`, a common mistake.
