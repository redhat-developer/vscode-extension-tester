...
- `User/locale.json` — declares which locale to use (e.g. `{ "locale": "ru", "osLocale": "ru" }`)
- `languagepacks.json` — registry mapping locale codes to installed language pack extensions

When you pass the `--locale` / `-L` flag, ExTester writes `User/locale.json` automatically. `languagepacks.json` works differently: it is generated by the VS Code CLI when the language pack is installed into the test instance (via `-i` or an install-from-marketplace step). ExTester preserves that file across its settings-directory reset, but never generates it itself — which is why the language pack must be installed through ExTester rather than with a system `code` binary. ExTester also passes `--locale=<locale>` on the VS Code command line, which covers display-language behavior when `languagepacks.json` is missing.

All you need to do is declare the language pack as an extension dependency so it gets installed into the test instance.
...
| `-e <dir>` / `--extensions_dir <dir>` | Directory where extensions are installed (optional)                                              |

> **Note:** `-L` requires the language pack to already be installed into the test instance — otherwise VS Code falls back to English. Passing `-i` alongside `-L` is the convenient way to do that via `extensionDependencies`, but `extest install-from-marketplace` or the programmatic `installFromMarketplace` work equally well. Without `-L`, VS Code starts in English even if the pack is installed.
>
> `-e` is optional. If you split setup and run into two commands, pass the same `-e` to both.
...
1. Make sure the language pack is installed into the test instance — via `-i`, `extest install-from-marketplace`, or the programmatic `installFromMarketplace`.
