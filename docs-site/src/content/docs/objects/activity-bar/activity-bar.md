...
![activityBar](../../../../assets/activity-bar.png)
...
Find a global actions button by title, returns a single control or `undefined` if not found

```typescript
const action = await activityBar.getGlobalAction("Manage");
```

## Open context menu

Right click on the activity bar to open the context menu
