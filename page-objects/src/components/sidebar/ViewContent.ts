...
export class ViewContent extends AbstractElement {
    ...
    async getSection<T extends ViewSection>(titleOrPredicate: string | ((section: ViewSection) => boolean | PromiseLike<boolean>), type?: ViewSectionConstructor<T>): Promise<T> {
        const sections = await this.getSections();
        const predicate = typeof titleOrPredicate === 'string' ? 
            (async (section: ViewSection) => await section.getTitle() === titleOrPredicate) : titleOrPredicate;
